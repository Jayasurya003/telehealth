...
const PatientMessages: React.FC = () => {
  console.log('PatientMessages mounted');
  ...
  // Add state to track read conversations
  ...
  // Fetch all messages and group by doctor/appointment
  useEffect(() => {
    console.log('PatientMessages userId:', userId);
    if (!userId) return;
    setLoading(true);
    api.get('/messages/').then(res => {
      console.log('DEBUG: /messages/ API response:', res.data);
      const grouped: { [key: string]: any } = {};
      res.data.forEach((msg: any) => {
        const appt = appointments.find(a => a.id === msg.appointment_id && a.patient && a.patient.user && a.patient.user.id === userId);
        if (appt) {
  ...
  // On mount, check for selectedConvId in sessionStorage
  useEffect(() => {
    if (conversations.length === 0) return;
    const selId = sessionStorage.getItem('selectedConvId');
    if (selId) {
      const found = conversations.find(conv => String(conv.appointment.id) === selId);
  ...
  useEffect(() => {
    if (!userId) return;
    const key = `patient_read_convs_${userId}`;
  ...
  useEffect(() => {
    if (selectedConv && userId) {
      ...
      setReadConvs(prev => {
        ...
        localStorage.setItem(`patient_read_convs_${userId}`, JSON.stringify(Array.from(updated)));
        return updated;
      });
    }
  }, [selectedConv, userId]);
  ...
  return (
    <Container fluid className="py-4" style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #e0eafc 0%, #cfdef3 100%)' }}>
      <Row>
          ...
          {loading ? <Spinner animation="border" /> : (
            <ListGroup>
              ...
              {conversations.map((conv, idx) => (
                ...
                >
                  <div className="rounded-circle bg-primary text-white d-flex align-items-center justify-content-center me-3" style={{ width: 40, height: 40, fontWeight: 600, fontSize: 18 }}>
                    {conv.doctor.user.name.charAt(0)}
                  </div>
                  <div className="flex-grow-1">
                    <div className="fw-semibold">Dr. {conv.doctor.user.name}</div>
        ...
        <Col md={8}>
          <h5 className="fw-bold mb-3">{selectedConv ? `Chat with Dr. ${selectedConv.doctor.user.name}` : 'Select a conversation'}</h5>
                    ...
                    return (
                      <div key={idx} className={`d-flex ${isMe ? 'justify-content-end' : 'justify-content-start'} mb-2`}>
                        <div style={{ maxWidth: '70%' }}>
                          <div className={`px-3 py-2 rounded-4 ${isMe ? 'bg-primary text-white' : 'bg-white border'}`} style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
